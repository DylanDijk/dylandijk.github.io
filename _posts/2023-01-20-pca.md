...
The definition of PCA is given so that the vectors are chosen iteratively, a natural question is then: could we get a set of orthonormal vectors such that the projected data has a larger total variance if we selected them directly?

It happens to be that solutions to both formulations are the same. So finding the first $d$ PC loadings iteratively, is equivalent to finding the orthogonal matrix $V$ such that:
